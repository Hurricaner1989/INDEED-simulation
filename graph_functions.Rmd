...
for (iter in 1:5) {
  ...
  indeed_precision_list <- append(indeed_precision_list, metrics_indeed_no_fdr$byClass[5])
  indeed_recall_list <- append(indeed_recall_list, metrics_indeed_no_fdr$byClass[6])
  ...
  metrics_indeed_fdr <- confusionMatrix(factor(label_indeed_fdr, levels = c('1', '0')), 
                                        factor(label_true, levels = c('1', '0')))
  # metrics_indeed_fdr
  indeed_fdr_precision_list <- append(indeed_fdr_precision_list, metrics_indeed_fdr$byClass[5])
  indeed_fdr_recall_list <- append(indeed_fdr_recall_list, metrics_indeed_fdr$byClass[6])
  ...
  metrics_dnapath <- confusionMatrix(factor(label_dnapath, levels = c('1', '0')), 
                                     factor(label_true, levels = c('1', '0')))
  # metrics_dnapath
  dnapath_precision_list <- append(dnapath_precision_list, metrics_dnapath$byClass[5])
  dnapath_recall_list <- append(dnapath_recall_list, metrics_dnapath$byClass[6])
  ...
  dnapath_auc_list <- append(dnapath_auc_list, pr_curve_dnapath$auc.davis.goadrich)
  toc(log = TRUE, quiet = TRUE)
  #### Remove unnecessary variables
  ...
  metrics_jdinac <- confusionMatrix(factor(label_jdinac, levels = c('1', '0')), 
                                    factor(label_true, levels = c('1', '0')))
  # metrics_jdinac 
  jdinac_precision_list <- append(jdinac_precision_list, metrics_jdinac$byClass[5])
  jdinac_recall_list <- append(jdinac_recall_list, metrics_jdinac$byClass[6])
...
timer <- tic.log(format = T)
```

```{r}
boxplot(indeed_recall_list, indeed_fdr_recall_list, dnapath_recall_list, jdinac_recall_list, main = 'Recall n = 25, p = 100', names = c('Indeed', 'Indeed_FDR', 'DNAPATH', 'JDNAC'))

boxplot(indeed_precision_list, indeed_fdr_precision_list, dnapath_precision_list, jdinac_precision_list, main = 'Precision n = 25, p = 100', names = c('Indeed', 'Indeed_FDR', 'DNAPATH', 'JDNAC'))

boxplot(indeed_auc_list, indeed_auc_list, dnapath_auc_list, jdinac_auc_list, main = 'AUC n = 25, p = 100', names = c('Indeed', 'Indeed_FDR', 'DNAPATH', 'JDNAC'))

# input time manually
# indeed_time <- c(106.507, 108.517, 108.206, 110.8, 109.107)
# indeed_fdr_time <- c(109.076, 109.024, 109.536, 108.802, 109.439)
# dnapath_time <- c(1.584, 1.706, 1.652, 1.543, 1.523)
# jdnac_time <- c(35.244, 35.084, 35.429, 34.747, 34.215)

boxplot(indeed_time, indeed_fdr_time, dnapath_time, jdnac_time, main = 'Run Time n = 25, p = 100', names = c('Indeed', 'Indeed_FDR', 'DNAPATH', 'JDNAC'), ylab = 'Seconds')
